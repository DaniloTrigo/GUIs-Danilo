...
/* -------- Desenho -------- */
static void desenharCena(SDL_Renderer* ren, int x) {
    ...
    stringRGBA(ren, 10, 10, "Pressione R para inverter a direcao", 0, 0, 0, 255);
...
int main(int argc, char** argv) {
    ...
    while (running) {
        int got = AUX_WaitEventTimeoutCount(&evt, &espera);

        if (got) {
            if (evt.type == SDL_KEYDOWN) {
                if (evt.key.keysym.sym == SDLK_ESCAPE) running = 0;
                if (evt.key.keysym.sym == SDLK_r) dx = -dx; // único evento: inverter direção
            }
